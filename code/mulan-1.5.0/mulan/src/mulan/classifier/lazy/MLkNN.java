...
public class MLkNN extends MultiLabelKNN {//定义一个MLKNN的公共类

    /**
     * Smoothing parameter controlling the strength of uniform prior <br> 光滑参数控制均匀先验的强度
     * (Default value is set to 1 which yields the Laplace smoothing). 设置为0，表示为拉普拉斯
     */
    protected double smooth; //光滑参数
    /**
     * A table holding the prior probability for an instance to belong in each
     * class 保存实例属于每个类的先验概率的表
     */
    private double[] PriorProbabilities; // 先验概率
    /**
     * A table holding the prior probability for an instance not to belong in
     * each class  保存实例不属于每个类的先验概率的表
     */
    private double[] PriorNProbabilities; // 先验非概率
    /**
     ...
     * class<br> given that i:0..k of its neighbors belong to that class
		一个表保存一个实例属于每个类的概率的表 - 假设其邻居的i：0..k属于该类
     */
    private double[][] CondProbabilities; //后验概率
    /**
     * A table holding the probability for an instance not to belong in each
     * class<br> given that i:0..k of its neighbors belong to that class
		一个表保存一个实例不属于每个类的概率的表 - 假设其邻居的i：0..k属于该类
     */
    private double[][] CondNProbabilities; //后验非概率

    /**
     * @param numOfNeighbors : the number of neighbors 参数：近邻数
     * @param smooth : the smoothing factor 参数：光滑因子
     */
    public MLkNN(int numOfNeighbors, double smooth) { //本类的构造器，传入指定的参数
    ...
    @Override
    protected void buildInternal(MultiLabelInstances train) throws Exception { // 保护函数，赋值一些参数
     ...
     * set
     */
    private void ComputePrior() { // 私有函数，计算先验概率
        for (int i = 0; i < numLabels; i++) {
            int temp_Ci = 0;
     ...
     */
    private void ComputeCond() throws Exception { // 私有函数，计算后验概率
    ...
    protected MultiLabelOutput makePredictionInternal(Instance instance) throws Exception { // 保护函数，对新数据做出预测
